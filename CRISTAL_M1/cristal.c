...
/*
 *Cheek that cristal is not already running!!
 */
    FILE* flocked = fopen("myflocked.txt", "a");

    int result = flock(fileno(flocked), LOCK_EX | LOCK_NB);
    if (result == -1)
    {
        printf("CRISTAL already running\n");
        exit (EXIT_FAILURE);
    }
  ...
  STInfo.istatus=ST_STATUS_TRIGGERING; 
 ...
 //Add a Thread for L3 
  ...
  while(True)
	...
	{             
	  transfer(0xA4, 0, 0, 0);
	  printf("Stopping Mono Global triggers.");
	
	  STInfo.istatus=ST_STATUS_STOPPED; 
	}
	...
	{                        
	  transfer(0xA3, 0, 0, 0);
          printf("Starting Mono Global triggers.");
	  STInfo.istatus=ST_STATUS_TRIGGERING; 
	}
    ...
    if(strcmp(verb,"READY")==0)
    {
      STInfo.istatus=ST_STATUS_TRIGGERING;
      TERM_BLUE_COLOR;
	printf("   __________  __________________    __ \n"); 
	printf("  / ____/ __ \\/  _/ ___/_  __/   |  / / \n"); 
	printf(" / /   / /_/ // / \\__ \\ / / / /| | / /  \n"); 
	printf("/ /___/ _, _// / ___/ // / / ___ |/ /___ \n");
	printf("\\____/_/ |_/___//____//_/ /_/  |_/_____/ \n");
	printf("                                     \n");
	printf("                                     by jrg\n");
     TERM_RESET_COLOR;
      sleep(2);
...
void exit_program() 
{
  ...
  error_logger(__FILE__,ERINFO,"SHUTTING DOWN ... ");
  STInfo.exit = True;
  STInfo.monexit=True;
