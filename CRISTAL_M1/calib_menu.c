...
*/
	...
	if(strcmp(verb,"CHEEKPIXELON")==0)
	{
	  ...
	  int macrocell_on[20];
	  int i,j,k,z,zz=0;
	  int pixel_l2[7]={3,6,8,9,10,15,19};
	  int pixel_l3[7]={1,5,10,11,12,14,17};
	  int clip_channel=0;
	  int data1,dat2;
	  int rate[101][55]={{-999}};//thresold-macrocell	
	  int Macrocell;
	 ...
	 for (k=1; k<4;k++)
	 {
		...
		if(k==1)
		{
			...
			{
                                channel=z+1;  
				(FUNC_CLIP_DIS ,ALL_BOARDS,ALL_C,0);
				  for (i=0;i<NumLines;i++)
				    {  
				      for (j=0;j<3;j++)
					{    
					  if (MAP[i].S_SUM_LAYER[j]==layer && MAP[i].S_SUM_CHANNEL[j]==channel)
					    {
					      clip_channel=(MAP[i].S_CLIP_CONNECTOR[j]-1)*8+MAP[i].S_CLIP_CHANNEL[j];
		                              pixel_on[zz]=MAP[i].PIXEL;
		                              macrocell_on[zz]=MAP[i].S_SUM_MACROCELL[j];
					      transfer(FUNC_CLIP_DIS , MAP[i].CLIP_SLOT,clip_channel-1, 1);
		                              zz++;
					    }
					}
		  	...
		  	for (j=1;j<56;j++)
		    	{
		     		...
		     		transfer_c(FUNC_ASTRO_TIM, &data1, &data2, &Macrocell);
		      		rate[k][j-1]=Macrocell;
		      		if (rate[k][j-1]>0.9*CalSet.Rate)
		      		{
		      			for(zz=0;zz<20;zz++)
					{
						if(j==macrocell_on[zz]) printf("Pixel %i ON",pixel_on[zz]);
