...
int histpos=-1,histfill=0;
char historial[MAX_HIST][FBUFSIZ];
char esclef[]={0x1b,0x5b,0x44,0};
char escrig[]={0x1b,0x5b,0x43,0};
char escdel[]={0x1b,0x5b,0x50,0};
char escins[]={0x1b,0x5b,0x40,0};
char escclear[]={0x1b,0x5b,0x4B,0};

//int2 AllowInput=0;

char *gethists (char *str)
{
  int c=0;
  int cnt,dastrpos=0;
  int status=0;
  ...
  histsel=(histpos+1)&MASK_HIST;
  dststr=historial[histsel];
  dststr[dastrpos]=0;
  while (c!=0x0A){
    c=getchar();
    switch (c){
    case 0x1B: 
      if (status&GETH_ALT) status|=GETH_ESC;
      else                 status|=GETH_ALT;
      break;
    case 0x5B:
      if ((status&GETH_ALT) && !(status&GETH_ESC)) status=GETH_ESC;
      break;
// >>>>> changed by M. Fras, 27 Feb 2009 >>>>>
//    case 0x4F:
//      if (status&GETH_ALT) status=GETH_FUN;
//      break;
// <<<<< changed by M. Fras, 27 Feb 2009 <<<<<
    case 0xC2: case 0xC3:  case 0xC4:  case 0xC5: status|=GETH_ACC; break;
    case 0xE2: status|=GETH_ESP; break;
    default:
      if (status&GETH_ESCW) {if (c==0x7E) status=0;}
      else if (status&GETH_ESP) status=GETH_ACC;
      else if (status&GETH_ACC)	status=0;
      else if (status&GETH_ESC) {
	switch (c){
	case 0x41:
	  status=0;
	  if (((histpos-histsel+1)&MASK_HIST)<histfill){
	    histsel=(histsel-1)&MASK_HIST;
	    dststr=historial[histsel];
	    for (cnt=0;cnt<dastrpos;cnt++) printf("%s",esclef);
	    printf("%s%s",escclear,dststr);   
	    dastrpos=strlen(dststr);    
	  }
	  
	  break;
	case 0x42:
	  status=0;
	  if (((histpos-histsel+1)&MASK_HIST)>0){
	    histsel=(histsel+1)&MASK_HIST;
	    dststr=historial[histsel];
	    for (cnt=0;cnt<dastrpos;cnt++) printf("%s",esclef);
	    printf("%s%s",escclear,dststr);
	...
	case 0x44: 
	  ...
	  break; //Cursor left
	case 0x46: case 0x48: status=0; break;
	case 0x33: 
	  status=GETH_ESCW;
	  strncpy(dststr+dastrpos,dststr+(dastrpos+1),1023-dastrpos);
	  printf("%s",escdel);
	  break; //Delete
	default: status=GETH_ESCW; break;
	}
      }
      else if (status&GETH_FUN) {
	if (c==0x32)  status|=GETH_SFT;
	else status=0; // Fkeys from 1 to 4
      }
      else if (c==0x0A) { printf("%c",c); status=0; }// Enter key!
      else if (c==0x7F) { // backspace :P
	if (dastrpos>0) {
	  strncpy(dststr+(dastrpos-1),dststr+dastrpos,FBUFSIZ-dastrpos);
	  dastrpos--;
	  printf("%s%s",esclef,escdel);
	}
      }
      else {
	if (dastrpos<1023){
	  status=0;
	  printf("%s%c",escins,c);
	  for (cnt=1023;cnt>dastrpos;cnt--) dststr[cnt]=dststr[cnt-1];
	  dststr[dastrpos++]=c;
	}
      }
      break;
  ...
  if (dststr[0]){
    histpos=(histpos+1)&MASK_HIST;
    if (histfill<(MAX_HIST-1)) histfill++;
    strncpy(historial[histpos],dststr,FBUFSIZ);
  }
  strncpy(str,dststr,FBUFSIZ);
  return(str);
}
...
int4 menu_delims( char *snew)
{
	if(snew!=NULL)
	  {
	    if(strlen(snew)!=strlen(delims))
	    {
			free(delims,"delims","menu_delims");
			delims=(char *)malloc(sizeof(char)*(strlen(snew)+1),"delims","menu_delims");
	    }
	    strcpy(delims,snew);
	  }
	return 0;
...
{
	return get_short_data( prompt, (short *) ivalue, low, high, "%X");
}
...
{
	return get_int_data( prompt, (long *) ivalue, low, high, "%08lX");
}
...
//	if (strcmp("? ",menuString)) error_logger(__FILE__,1,menuString);
  return 0;
}
